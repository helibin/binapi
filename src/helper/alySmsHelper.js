 ...
 * @Last Modified time: 2020-02-28 11:07:27
 */
/** 内建模块 */
...
const alyAk = smsConf.accessKeyId || CONFIG.alyServer.accessKeyId
const alyAs = smsConf.accessKeySecret || CONFIG.alyServer.accessKeySecret
const signName = smsConf.signName
let smsClient = {}

try {
  if (alyAk && alyAs && signName) {
    smsClient = new Sms({ accessKeyId: alyAk, secretAccessKey: alyAs })
  } else {
    Log.logger('error', 'alySms服务缺少参数  : ', `alyAk='${alyAk}', alyAs='${alyAs}', signName=${signName}`)
  }
} catch (ex) {
...
export default class {
  ...
  createCacheKey(cate, clientId, phone) {
    return `smsCode@${cate}#user=${CONFIG.webServer.name}:type=aliSms:clientId=${clientId}:phone=${phone}`
  }
  ...
  async sendSms(phone, cate = 'default', len = 4) {
    ...
    try {
      ...
      if (!this.t.isEmpty(retryCacheCheck)) {
        throw new ce('eAliyunAPI', 'requestFrequently')
      }
      ...
      if (!this.t.isEmpty(limitDayCacheCheck)) {
        if (Math.abs(dayjs().diff(limitDayCacheCheckObj.initTime, 'day'))) {
          ...
          throw new ce('eAliyunAPI', 'limitRequestPerDay')
        }
      }
      ...
      if (!this.t.isEmpty(limitHourCacheCheck)) {
        if (Math.abs(dayjs().diff(limitHourCacheCheckObj.initTime, 'hour'))) {
          ...
          throw new ce('eAliyunAPI', 'limitRequestPerHour')
        }
      }
